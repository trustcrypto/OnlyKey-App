...
var OnlyKeyHID = function (onlyKeyConfigWizard) {
    ...
    var handleMessage = async function (err, msg) {
        ...
        if (myOnlyKey.isBootloader || !myOnlyKey.isInitialized ) { //Firmware load in app without config mode
          ...
          step10text.innerHTML = "<form id='restoreForm' name='restoreForm' class='auth-form'><h2>Restore</h2><fieldset>To restore a backup file to your OnlyKey, ensure you have loaded the same key you used to create the backup and set the key as your backup key.<p>Click [Choose File], select your backup file, then click [Restore to OnlyKey].</p><p><u>Step 3</u>. The OnlyKey light will be yellow while restoring your backup, then will automatically reboot when restoring is complete.</p></fieldset><br/><br/><input type='file' id='restoreSelectFile' name='restoreSelectFile' value='Select File' /><br/><br/><input type='button' id='doRestore' name='doRestore' value='Restore to OnlyKey' /><div id='restoreFormError' class='form-error'></div></form>";
          ...
          step8text.innerHTML = "To set a new passphrase on your OnlyKey, Hold down the #6 button on your OnlyKey for 5+ seconds and release. The OnlyKey light will turn off. Re-enter your PIN to enter config mode. Using the instructions below select a passphrase and submit to set the passphrase on your OnlyKey. When finished remove and reinsert OnlyKey.</p><p>Your passphrase will be used for secure backup and restore of your OnlyKey so make sure to choose a good one, write it down, and store it in a secure location.</p><p>Example of a <em>good</em> passphrase: 'this passphrase is not complex but it is long and it is not a common phrase'</p><p>Example of a <em>bad</em> passphrase: 'the only thing we have to fear is fear itself' </p><br/><label><b>Enter Passphrase:</b><input type='password' id='backupPassphrase' name='backupPassphrase' size=50 /><br>Passphrase must be at least 25 characters</label><br/><br/><label><b>Re-Enter Passphrase:</b><input type='password' id='backupPassphrasec' name='backupPassphrasec' size=50 /></label><br/><label><input type='radio' checked name='backupKeyMode' value=0 /><u>Permit future backup key changes (Default)</u></label><br/><label><input type='radio' name='backupKeyMode' value=1 /><u>Set backup key only once</u></label><br/><br/>Learn more about secure backup <a href='https://docs.crp.to/usersguide.html#secure-encrypted-backup-anywhere' class='external'>here</a>.";
          ...
          step9text.innerHTML = "This version of firmware is outdated and does not support this feature. To load latest firmware follow the loading instructions <a href='https://docs.crp.to/usersguide.html#loading-onlykey-firmware' class='external'>here</a>";
          step10text.innerHTML = "This version of firmware is outdated and does not support this feature. To load latest firmware follow the loading instructions <a href='https://docs.crp.to/usersguide.html#loading-onlykey-firmware' class='external'>here</a>";
        }
