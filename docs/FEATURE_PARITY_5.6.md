...
**Assessed:** 2026-07-19 (source inventory) · **Status review:** 2026-08-17 · **Firmware path:** 2026-09-12  
**Method:** Source inventory of `src/`, desktop scripts, packaging, and automated tests. **Done is not a hardware sign-off.** Firmware SHA-256 download and React dialog have unit/UI coverage; HID load has not been run on a real key. App auto-update has mocked/unit coverage only; it has not been run on a published 5.7 update channel.
...
| Firmware | **Unverified** (React dialog + SHA-256 download; HID bootloader stream not loaded onto Classic/DUO) |
...
| Automated tests | **Strong** (674 UI/unit tests in 75 files as of 2026-09-12; desktop NW tests separate) |
...
| Firmware during setup | Unverified | file pick + optional Download Latest / “Use downloaded” on Step 11 — same untested HID path as Firmware tab |
...
| Auto-check / alert | Unverified | `FirmwareUpdateHost` + `FirmwareUpdateDialog` (React, not `window.confirm`); Tools + tray; `autoUpdateFW` defaults **on** |
...
1. **Firmware load untested on hardware** — Classic and DUO file-load, bootloader kick, block stream, and resume have not been signed off. Auto path is dialog + SHA-256; hardware still **lab**. `autoUpdateFW` defaults on and will prompt from GitHub.
...
`npm run test:unit` (`vitest run --project ui`) on 2026-09-12: **674 tests / 75 files passed**. Desktop NW tests (`npm run test:desktop`) are separate and do not cover firmware load or app update.
